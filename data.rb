...
class Shield
  ...
  case option 
    when 1 
      ...
      $mission_details.each do |x|
       if x[:mission_avenger_name].length == 0
        puts "No mission is assigned"
       elsif x[:mission_avenger_name].length != 0
    ...
    when 4 
      check_avengers_details
    when 5
      update_status
    when 6
      list_avengers
    when 7
      assign_avnegers_to_mission
    end
  end
  ...
  def check_avengers_details
    puts "Anter Avengers Name"
    name = gets.chomp.to_s
    if $avengers_name.include?(name)
      $avengers_details.each do |avg_details|
        puts "Avengers Name = #{name}"
